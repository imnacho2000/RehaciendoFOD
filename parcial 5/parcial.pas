...
type
    ...
    procedure generar_informe(var archivoM: archivo_maestro; anio: string[25]);
    var
        ...
        while( (reg_m.anio <> anio ) and (reg_m.anio <> valor_corte )) do begin
            ...
            if(reg_m.anio = anio) then begin
                while(reg_m.anio <> valor_corte) do begin
                    ...
                    total_tiempo_mes:= 0;
                    while((anio_aux.anio = reg_m.anio) and (reg_m.mes = mes_aux)) do begin
                        tiempo_total_diario := 0;
                        write('Mes: ', mes_aux.mes);
                        dia_aux := reg_m.dia;
                        while((anio_aux.anio = reg_m.anio) and (reg_m.mes = mes_aux) and (reg_m.dia = dia_aux.dia)) do begin
                            tiempo_total_por_usuario:= 0;
                            write('Dia: ', reg_m.dia);
                            idUsuario_aux.idUsuario := reg_m.idUsuario;
                            write('Id usuario: ', reg_m.idUsuario);
                            while((anio_aux.anio = reg_m.anio) and (reg_m.mes = mes_aux) and (reg_m.dia = dia_aux.dia) and (reg_m.idUsuario = idUsuario_aux.idUsuario_aux)) do begin
                        ...
                        write('Tiempo total acceso dia: ', reg_m.dia,' Mes: ',reg_mes,' : ', tiempo_total_diario);
                        total_tiempo_mes:= total_tiempo_mes + tiempo_total_diario;
                    end;
                    write('Total tiempo de acceso: ', total_tiempo_mes, ' Mes:', reg_m.mes);
                    total_anio:= total + total_tiempo_mes;
                end;
                write('Total tiempo de acceso ano: ', total_anio);
            end
            else
